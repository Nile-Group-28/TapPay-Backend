...
router.post('/register', registerRules, handleValidation, async (req, res) => {
  ...
  log.auth(`Register — name:"${name}" phone:${phone || '-'} email:${email || '-'}`);

  if (!email && !phone) {
    return res.status(400).json({ success: false, message: 'Provide an email or phone number.' });
  }
  ...
  try {
    ...
    const [pinHash, pwHash] = await Promise.all([
      bcrypt.hash(pin, 12),
    ...
    log.ok(`Register SUCCESS — user:${user.id}`);
    ...
    return res.status(201).json({
      ...
      user: {
        ...
        role: user.role, kycTier: user.kyc_tier, kycStatus: user.kyc_status,
        balance: 0, hasPassword: !!pwHash,
      },
    });
  } catch (err) {
    await client.query('ROLLBACK');
...
router.post('/login', loginRules, handleValidation, async (req, res) => {
  ...
  if (!pin && !password) {
    return res.status(400).json({ success: false, message: 'PIN or password is required.' });
  }
  log.auth(`Login — identifier:${identifier} method:${pin ? 'PIN' : 'password'}`);
  ...
  try {
    ...
    let match = false;
    if (pin && user.pin_hash)             match = await bcrypt.compare(pin, user.pin_hash);
    ...
    const isNewDevice = !!(deviceId && user.device_id && user.device_id !== deviceId);
    if (isNewDevice) {
      await pool.query(
        `UPDATE users SET failed_login_attempts=0, locked_until=NULL,
           device_id=$1, device_registered_at=NOW(), updated_at=NOW() WHERE id=$2`,
        [deviceId, user.id]
      );
      await pool.query(
        `INSERT INTO security_logs (user_id, event_type, ip_address, device_id, note)
         VALUES ($1,'NEW_DEVICE_LOGIN',$2,$3,$4)`,
        [user.id, req.ip, deviceId, `Previous: ${user.device_id}`]
      );
    } else {
      await pool.query(
    ...
    await pool.query(
      `INSERT INTO security_logs (user_id, event_type, ip_address, device_id) VALUES ($1,'LOGIN',$2,$3)`,
      [user.id, req.ip, deviceId || null]
    );

    log.ok(`Login SUCCESS — user:${user.id}`);
    const token = jwt.sign(
      { userId: user.id, role: user.role },
      process.env.JWT_SECRET,
      { expiresIn: process.env.JWT_EXPIRES_IN || '7d' }
    ...
    return res.json({
      ...
      user: {
        ...
        isBiometricsEnabled: user.is_biometrics_enabled,
        isNewDevice, hasPassword: !!user.password_hash,
      },
    });
  } catch (err) {
...
router.post('/change-pin', authenticate, async (req, res) => {
  ...
  if (!currentPin || !newPin || !/^\d{4,6}$/.test(newPin)) {
    return res.status(400).json({ success: false, message: 'Both PINs required; new PIN must be 4–6 digits.' });
  }
  try {
    const { rows } = await pool.query('SELECT pin_hash FROM users WHERE id=$1', [req.user.id]);
    if (!rows.length || !(await bcrypt.compare(currentPin, rows[0].pin_hash))) {
    ...
    await pool.query(
      `INSERT INTO security_logs (user_id, event_type, ip_address) VALUES ($1,'PIN_CHANGED',$2)`,
      [req.user.id, req.ip]
    );
...
router.post('/set-password', authenticate, async (req, res) => {
  ...
  if (!currentPin) {
    return res.status(400).json({ success: false, message: 'Current PIN is required to set a password.' });
  }
  try {
    const { rows } = await pool.query('SELECT pin_hash FROM users WHERE id=$1', [req.user.id]);
    if (!rows.length || !(await bcrypt.compare(currentPin, rows[0].pin_hash))) {
    ...
    await pool.query(
      `INSERT INTO security_logs (user_id, event_type, ip_address) VALUES ($1,'PASSWORD_SET',$2)`,
      [req.user.id, req.ip]
    );
  ...
  try {
    ...
    return res.json({
      ...
      user: {
        ...
        role: r.role, kycTier: r.kyc_tier, kycStatus: r.kyc_status,
        balance: parseFloat(r.balance || 0),
        isBiometricsEnabled: r.is_biometrics_enabled,
