...
router.post('/activate', authenticate, async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ success: false, message: 'Failed to activate POS.' });
  }
});
...
router.post('/transaction', authenticate, async (req, res) => {
  ...
  if (totalAmount <= 0) return res.status(400).json({ success: false, message: 'Total must be positive.' });
  ...
  try {
    ...
    if (!debitRes.rows.length) {
      ...
      return res.status(400).json({ success: false, message: 'Insufficient funds or wallet is locked.' });
    }
...
router.get('/daily-report', authenticate, async (req, res) => {
  ...
  try {
    const [summaryRes, refundRes, hourlyRes, topItemsRes] = await Promise.all([
      pool.query(
        `SELECT
      ...
      pool.query(
        `SELECT COALESCE(SUM(r.refund_amount), 0) AS total_refunds
         FROM pos_refunds r WHERE r.merchant_id = $1 AND DATE(r.created_at) = $2`,
        [req.user.id, date]
      ),
      pool.query(
...
router.get('/transactions', authenticate, async (req, res) => {
  ...
  try {
    ...
    const whereClauses = [`t.receiver_id = $1`, `t.type IN ('NFC','QR')`];

    if (startDate) { whereParams.push(startDate); whereClauses.push(`DATE(t.created_at) >= $${whereParams.length}`); }
    if (endDate)   { whereParams.push(endDate);   whereClauses.push(`DATE(t.created_at) <= $${whereParams.length}`); }
    if (status)    { whereParams.push(status.toUpperCase()); whereClauses.push(`t.status = $${whereParams.length}`); }
    ...
    log.error('POS transactions error', err.message);
...
router.post('/refund', authenticate, async (req, res) => {
  ...
  try {
    ...
    const daysSince = (Date.now() - new Date(original.created_at).getTime()) / 86400000;
    if (daysSince > MAX_REFUND_DAYS) {
      return res.status(403).json({ success: false, message: `Refunds only allowed within ${MAX_REFUND_DAYS} days.` });
    }

    if (parsedRefund > parseFloat(original.amount)) {
      return res.status(400).json({ success: false, message: `Refund cannot exceed original amount of ₦${original.amount}.` });
    }
    ...
    const refundTxRes = await client.query(
      ...
      [req.user.id, original.sender_id, customerCredit.rows[0].id,
       parsedRefund, original.type, `Refund for tx ${transactionId.substring(0, 8)}`]
    );
    const refundTxId = refundTxRes.rows[0].id;

    await client.query(
      `INSERT INTO pos_refunds
         (original_transaction_id, refund_transaction_id, merchant_id,
          refund_amount, refund_reason, refund_type, processed_by)
...
router.get('/settlement-report', authenticate, async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ success: false, message: 'Failed to generate settlement report.' });
  }
});
  ...
  try {
    const result = await pool.query(
      `INSERT INTO merchant_inventory
         (merchant_id, sku, item_name, category, price, stock_quantity, low_stock_alert)
       VALUES ($1,$2,$3,$4,$5,$6,$7)
       ...
       RETURNING *`,
      [req.user.id, sku, itemName, category, parseFloat(price),
       parseInt(stockQuantity) || 0, parseInt(lowStockAlert) || 5]
    );
...
router.get('/inventory', authenticate, async (req, res) => {
  ...
  try {
    const whereClauses = ['merchant_id = $1', 'is_active = true'];
    const params = [req.user.id];
    if (search)             { params.push(`%${search}%`); whereClauses.push(`(item_name ILIKE $${params.length} OR sku ILIKE $${params.length})`); }
    if (lowStock === 'true')  whereClauses.push('stock_quantity <= low_stock_alert');

    const result = await pool.query(
      `SELECT * FROM merchant_inventory WHERE ${whereClauses.join(' AND ')} ORDER BY item_name`,
      params
    );
